...
const teamp = require("../models/points");
...
router.get("/", async (req, res) => {
  const wof = await teamp.find({}).sort({ createdAt: -1 });
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    let info = await transporter.sendMail({
      ...
      subject: "New Team Added ", // Subject line
      text: `New Team : ${teamn}`, // plain text body
      html: "",
    });
    console.log("Message sent: %s", info.messageId);
    console.log("Message sent: %s", nodemailer.getTestMessageUrl(info));
  } catch (err) {
    res.status(404).json({ err: "err in team" });
  }
});
